...
import { TSignInResponse } from "../types/auth/signIn.type";
...
export const resolveResponse = (response: any) => {
    ...
    if(response.status >= 400 && response.status < 500) {
        if(response.status === 401) {
            toast.warn("Sessão finalizada!", {theme: 'colored'});
...
export const saveLocalStorage = (data: TSignInResponse, hasToken: boolean = false) => {
    if(hasToken) {
